...
export default tool({
  description:
    "Read the contents of a file, returned with line numbers. REQUIRED: " +
    "`filePath` — the path to the file (e.g. `filePath=\"calc.py\"`). OPTIONAL " +
    "but RECOMMENDED for anything but a tiny file: read a bounded RANGE instead " +
    "of the whole file by passing `offset` (the 1-based line number to start " +
    "at) and `limit` (how many lines to read). Worked example: to read lines " +
    "40-60 of a file, call read with offset=40 and limit=20. The output ends " +
    "with a footer telling you the offset to pass next to continue paging. " +
    "Reading a narrow range keeps each turn small — always grep first to find " +
    "the line you need, then read a small window around it.",
  args: {
    filePath: tool.schema
      .string()
      .describe("REQUIRED. The path to the file to read, e.g. \"calc.py\" or " +
        "an absolute path."),
    offset: tool.schema
      .number()
      .optional()
      .describe("OPTIONAL. The 1-based line number to start reading from. Pair " +
        "with `limit` to read a specific range, e.g. offset=40 to start at line 40."),
    limit: tool.schema
      .number()
      .optional()
      .describe("OPTIONAL. The maximum number of lines to read from `offset`, " +
        "e.g. limit=20 to read 20 lines. Keep this small to save context."),
  },
  async execute(args) {
